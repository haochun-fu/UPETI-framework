...
class DINWrapper(WrapperBase):
    ...
    def load(self, model_dir: str) -> None:
        ...
        file = os.path.join(model_dir, DINWrapper.MODEL_FILENAME)
        self._model = load_model(file, custom_objects)

        file = os.path.join(model_dir, DINWrapper.ENCODERS_FILENAME)
        self.encoders = CommFunc.load_pickle(file)
    ...
    def save(self, model_dir: str) -> None:
        ...
        file = os.path.join(model_dir, DINWrapper.MODEL_FILENAME)
        save_model(self._model, file)

        file = os.path.join(model_dir, DINWrapper.ENCODERS_FILENAME)
        CommFunc.save_data(self.encoders, file, save_type='pickle')

        file = os.path.join(model_dir, DINWrapper.TRAIN_HISTORY_FILENAME)
        CommFunc.save_data(self.__train_history, file, save_type='json')
